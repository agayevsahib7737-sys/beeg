 ...
 */
export function createRedisJobStorage(options) {
    ...
    return {
        ...
        async replayFailedJob(input, _options) {
            let replayedJobId = crypto.randomUUID();
            let now = Date.now();
            let result = await evalScript(redis, REPLAY_FAILED_JOB_SCRIPT, [
        ...
        async replaceSchedules(input) {
            ...
            for (let schedule of input) {
                await evalScript(redis, UPSERT_SCHEDULE_SCRIPT, [keys.schedule(schedule.id), keys.schedulesDue], [
                    schedule.id,
                    schedule.schedule,
                    schedule.timezone,
                    schedule.queue,
        ...
        async claimDueSchedules(input) {
            if (input.limit <= 0) {
                return [];
            }
            let result = await evalScript(redis, CLAIM_DUE_SCHEDULES_SCRIPT, [keys.schedulesDue, keys.schedulePrefix], [String(input.now), input.workerId, String(input.leaseMs), String(input.limit)]);
        ...
        async advanceSchedule(input) {
            await evalScript(redis, ADVANCE_SCHEDULE_SCRIPT, [keys.schedule(input.scheduleId), keys.schedulesDue], [input.scheduleId, input.workerId, String(input.nextRunAt), String(input.now)]);
        },
    };
}
...
async function getSchedule(redis, keys, scheduleId) {
...
async function zrange(redis, key, start, end) {
