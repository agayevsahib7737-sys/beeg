 ...
 */
export function createDataTableJobStorage(options) {
    ...
    return {
        ...
        replayFailedJob(input, writeOptions) {
            let db = writeOptions?.transaction ?? baseDb;
            return runOperation(() => db.transaction(async (database) => {
        ...
        claimDueJobs(input) {
            ...
            return runOperation(() => options.db.transaction(async (database) => {
        ...
        heartbeat(input) {
            return runOperation(async () => {
                let result = await options.db.exec(rawSql([
        ...
        replaceSchedules(input) {
            return runOperation(() => options.db.transaction(async (database) => {
                let desiredScheduleIds = Array.from(new Set(input.map((schedule) => schedule.id)));
                for (let schedule of input) {
                    let existingRows = await database.exec(rawSql(`select next_run_at from ${tables.schedules} where id = ? limit 1`, [schedule.id]));
                ...
                await database.exec(rawSql(`delete from ${tables.schedules} where id not in ${desiredScheduleInClause}`, desiredScheduleIds));
            }));
        },
        claimDueSchedules(input) {
            return runOperation(() => options.db.transaction(async (database) => {
                let dueIds = await database.exec(rawSql([
                    `select id from ${tables.schedules} where next_run_at <= ? `,
                    'and (locked_until is null or locked_until <= ?) ',
                    'order by next_run_at asc limit ?',
                ].join(''), [input.now, input.now, input.limit]));
                let dueSchedules = [];
                for (let row of dueIds.rows ?? []) {
                    if (typeof row.id !== 'string') {
                        continue;
                    }
                    let update = await database.exec(rawSql([
                    ...
                    ].join(''), [input.workerId, input.now + input.leaseMs, input.now, row.id, input.now, input.now]));
                    if ((update.affectedRows ?? 0) === 0) {
                        continue;
                    }
                    let scheduleRows = await database.exec(rawSql(`select * from ${tables.schedules} where id = ? limit 1`, [row.id]));
        ...
        advanceSchedule(input) {
            return runOperation(async () => {
                ...
                ].join(''), [input.nextRunAt, input.now, input.scheduleId, input.workerId]));
            });
        },
    };
}
