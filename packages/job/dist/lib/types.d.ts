...
export type CronSchedule<defs extends JobDefinitions, name extends JobName<defs> = JobName<defs>> = {
    schedule: string;
    job: JobReference<defs, name>;
    payload: Infer<defs[name]['schema']>;
...
export interface JobScheduler<defs extends JobDefinitions, transaction = never> {
    ...
    replayFailedJob(jobId: string, options?: ReplayFailedJobOptions<transaction>): Promise<ReplayFailedJobResult>;
...
export type CreateJobSchedulerOptions<defs extends JobDefinitions, transaction = never> = {
    jobs: defs;
