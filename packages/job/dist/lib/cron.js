...
export function getNextCronRunAt(cron, fromMs, timeZone = 'UTC') {
...
export function getCronDispatchCount(cron, timeZone, catchUp, nextRunAt, now) {
    if (catchUp === 'none') {
        return 0;
    }
    if (catchUp === 'one') {
...
function parseField(field, minValue, maxValue, normalizeSunday) {
    if (field === '*') {
        return null;
    }
    let values = new Set();
...
function parseToken(token, minValue, maxValue, normalizeSunday, output) {
...
function addRange(output, start, end, step, normalizeSunday, minValue, maxValue) {
...
function addValue(output, value, normalizeSunday, minValue, maxValue) {
...
function matchesValue(values, value) {
    if (values == null) {
        return true;
    }
